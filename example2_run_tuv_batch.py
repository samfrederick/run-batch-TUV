...
"""
In example 2, we're varying two dimensions - the first 'iterable_i' is the same 
as example 1, letting the month vary from Jan. to Dec. The second dimension 
'iterable_j' is now latitude ranging from -90 to 90 in steps of 10 degrees.

Everything else is the same across simulation runs.
"""
...
# create an array for iterating over latitudes 'lat_range'. This is passed to 
# the 'lat' argument in batch_run() below.
lat_spacing = 10
lat_min = -90
lat_max = 90
lat_range = np.arange(lat_min, lat_max + 1, lat_spacing)

batch_run(data_subdir='example-2-output',
